...
class LSPI:
    ...
    def __init__(self, environment, noF, bandwidth, exploration_rate = 1, discount_factor = 0.95, epsilon = 0.0000001):
        ...
        self.actions = [[5.0, 5.0], [5.0, -5.0], [-5.0, 5.0], [-5.0, -5.0]]
        ...
        self.allFeatures = self.get_feature_fun(8, self.numberOfFeatures, bandwidth)
    ...
    def LSPI_algorithm(self, firstAction_id = 0, training_samples = 200, maxTimeSteps = 2000):
        ...
        while doneActions < 100000:
            ...
            for t in range(maxTimeSteps):
                ...
                obs, reward, done, _ = self.environment.step(np.array([self.actions[previousAction_id]])[0])
                doneActions += 1
    ...
    def apply(self):
        ...
        for n in range(50):
            ...
            for t in range(1000):
                obs, reward, done, _ = self.environment.step(np.array([self.actions[currentAction_id]])[0])
                allReward += reward
                if done:
                    break
                #self.environment.render()
...
env = gym.make('BallBalancerSim-v0')  # Use "Cartpole-v0" for the simulation
env.reset()
def main():
    ...
    for i in range(0, 15):
        ...
        print(i)
        xd = LSPI(env, 260 + i * 15, 17.3)
        xd.LSPI_algorithm()
        y = xd.apply()
        x_axis.append(300 + i*12)
        y_axis.append(y)
    plt.scatter(x_axis, y_axis)
    plt.show()

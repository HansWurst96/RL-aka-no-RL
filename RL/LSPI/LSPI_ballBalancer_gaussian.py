...
class LSPI:
    ...
    def __init__(self, environment, exploration_rate = 0.3, discount_factor = 0.95, epsilon = 0.01):
        ...
        self.numberOfFeatures = 3*3*3*3 + 1
        ...
        self.m_mu = []
        for theta_x in np.linspace(-np.pi/4, np.pi/4, 3):
                for theta_y in np.linspace(-np.pi/4, np.pi/4, 3):
                    for x in np.linspace(-0.15, 0.15, 3):
                        for y in np.linspace(-0.15, 0.15, 3):
                            mu = [theta_x, theta_y, x, y]
                            self.m_mu.append(mu)
